const challenges = {
    truths: {
        1: [
            { text: "T'chiche de me dire ta pire gaffe en public, avec tous les détails croustillants ?" },
            { text: "T'chiche de me révéler le surnom ridicule que ta famille t'a donné et pourquoi ?" },
            { text: "T'chiche de m'avouer la méthode la plus farfelue que tu as utilisée pour espionner un ex ?" },
            { text: "T'chiche de rejouer ta réaction quand tu m'as vu(e) pour la première fois ?" },
            { text: "T'chiche de me raconter ton mensonge le plus créatif pour éviter un rendez-vous ?" },
            { text: "T'chiche de me dire quel emoji représente parfaitement ta vie sexuelle actuelle ?" },
            { text: "T'chiche d'imiter l'animal qui te représente au lit ?" }
        ],
        2: [
            { text: "T'chiche de me dire quel plaisir coupable tu caches à tout le monde mais que tu assumes totalement ?" },
            { text: "T'chiche de me décrire le baiser parfait en utilisant uniquement des noms de desserts ?" },
            { text: "T'chiche de me raconter le mensonge le plus absurde que tu as inventé pour impressionner quelqu'un ?" },
            { text: "T'chiche de me dessiner avec ton doigt sur ma peau la zone qui te fait complètement perdre la tête ?" },
            { text: "T'chiche de me chuchoter le nom de la célébrité qui te fait fantasmer et pourquoi ?" },
            { text: "T'chiche de me révéler ta technique secrète pour séduire quelqu'un qui te plaît ?" },
            { text: "T'chiche de m'expliquer ton dernier rêve érotique en utilisant uniquement des métaphores culinaires ?" }
        ],
        3: [
            { text: "T'chiche de me raconter ton expérience la plus risquée dans un lieu public, avec les détails juteux ?" },
            { text: "T'chiche de me dire avec quelle personne célèbre tu voudrais passer une nuit torride et ce que vous feriez ?" },
            { text: "T'chiche de me mimer la pratique la plus exotique qui t'intrigue sans utiliser tes mains ?" },
            { text: "T'chiche de me susurrer ton fantasme le plus inavouable en changeant ta voix ?" },
            { text: "T'chiche de me décrire ton scénario de film érotique idéal où nous serions les personnages principaux ?" },
            { text: "T'chiche de m'expliquer comment tu me séduirais si on venait juste de se rencontrer ?" },
            { text: "T'chiche de me dire quel accessoire surprenant tu aimerais introduire dans nos jeux intimes ?" }
        ],
        4: [
            { text: "T'chiche de me décrire en détail la pensée la plus torride que tu as eue sur moi aujourd'hui, en fermant les yeux ?" },
            { text: "T'chiche de me murmurer trois mots qui te feraient immédiatement succomber si je te les disais pendant l'amour ?" },
            { text: "T'chiche de me révéler qui serait dominant et qui serait soumis dans ton fantasme idéal avec moi, et pourquoi ?" },
            { text: "T'chiche de me décrire l'expérience la plus folle que tu voudrais tenter avec moi si toutes les règles disparaissaient ?" },
            { text: "T'chiche de me dire dans quel endroit insolite tu rêves de me faire l'amour et dans quelle position ?" },
            { text: "T'chiche de me raconter ton fantasme le plus créatif impliquant de la nourriture et nos corps ?" },
            { text: "T'chiche de me détailler comment tu utiliserais une plume, de la glace et un bandeau avec moi ?" }
        ]
    },
    dares: {
        1: [
            { text: "T'chiche de me chuchoter un compliment à l’oreille." },
            { text: "T'chiche de me faire un câlin de 20 secondes, les yeux fermés." },
            { text: "T'chiche de dire un mot coquin avec ta voix la plus innocente." },
            { text: "T'chiche de me regarder dans les yeux sans parler pendant 30 secondes." },
            { text: "T'chiche de me faire un compliment physique." },
            { text: "T'chiche de me raconter un souvenir tendre que tu gardes pour toi." },
            { text: "T'chiche de me caresser doucement la main ou le bras pendant 10 secondes." }
        ],
        2: [
            { text: "T'chiche de me faire un massage sensuel d’une minute sur une zone de mon choix." },
            { text: "T'chiche de me demander où je rêve d’être touché·e maintenant, et de l’effleurer." },
            { text: "T'chiche de dessiner un mot secret sur mon dos avec ton doigt." },
            { text: "T'chiche de m’écrire un message de séduction avec ton doigt sur ma peau." },
            { text: "T'chiche de me raconter ton rêve érotique le plus marquant." },
            { text: "T'chiche de me chuchoter un secret excitant que personne ne connaît." },
            { text: "T'chiche de me caresser les cheveux ou le visage lentement, comme dans un film." }
        ],
        3: [
            { text: "T'chiche de me proposer un scénario de jeu de rôle que tu aimerais tester." },
            { text: "T'chiche d’enlever un vêtement de ton choix." },
            { text: "T'chiche de simuler un baiser à 1 cm de mes lèvres." },
            { text: "T'chiche de me faire deviner ta zone érogène préférée sans mots." },
            { text: "T'chiche de me chuchoter ce que tu veux que je fasse… sans le dire explicitement." },
            { text: "T'chiche de me guider pour toucher une zone précise de ton corps." },
            { text: "T'chiche de me faire deviner une scène coquine que tu aimerais vivre, version mime." }
        ],
        4: [
            { text: "T'chiche de me faire une dégustation de 3 endroits de mon corps et décrire leur goût." },
            { text: "T'chiche d’écrire un mot coquin avec ton doigt que je dois effacer avec ma bouche." },
            { text: "T'chiche de faire un strip-tease avec un accessoire." },
            { text: "T'chiche de me murmurer ce que tu veux me faire dans les moindres détails." },
            { text: "T'chiche de me bander les yeux et de me faire deviner 3 caresses." },
            { text: "T'chiche de me faire une mise en scène sexy comme si on était dans un film." },
            { text: "T'chiche de me prendre par la taille et de simuler une danse très sensuelle." }
        ]
    },
    gages: {
        1: [
            { text: "Le Gage Théâtral : \"La Déclaration Absurde\"\nLe joueur doit faire une déclaration d'amour passionnée et enflammée à l'objet le plus improbable de la pièce (une plante verte, une lampe, un coussin...). Il/elle doit être très convaincant(e), comme dans une grande scène de film romantique." },
            { text: "Le Gage Social : \"Le Community Manager d'un Soir\"\nLe joueur doit donner son téléphone au groupe. Les autres joueurs ont 1 minute pour écrire un statut ou une story courte (3-4 mots max, sans insulte) et complètement absurde sur le réseau social de leur choix (Instagram, Facebook, etc.). Le statut doit rester en ligne pendant au moins 15 minutes." },
            { text: "Le Mime Gourmand\nLe joueur doit mimer qu'il/elle mange son plat préféré de la manière la plus sensuelle possible pendant 30 secondes. Les autres joueurs doivent deviner quel est ce plat. Si personne ne trouve, le joueur doit continuer pendant 15 secondes supplémentaires." }
        ],
        2: [
            { text: "😈 Gage : L’Esclave de la Tentation\n🧎‍♂️ Tu deviens le ou la \"serviteur·se personnel·le\" du joueur de ton choix pendant 3 tours. Chaque fois qu’il ou elle te regarde, tu dois dire : Je suis à toi." },
            { text: "🎲 Gage : La Roulette Coquine\n🎰 Les autres joueurs choisissent au hasard un défi pour toi dans la liste." },
            { text: "🔥 Gage : Le Secret Confessé\n🤫 Révèle ton dernier mensonge ou un secret que tu n'as jamais osé dire à personne dans la pièce. Si tu refuses, tu dois répondre à 3 questions indiscrètes posées par les autres joueurs." }
        ],
        3: [
            { text: "Gage : Message Dating\n📱 Prends ton téléphone, les autres choisissent une lettre au hasard. Tu dois envoyer un SMS de proposition de date au premier contact de ton téléphone commençant par cette lettre, à voix haute." },
            { text: "Gage : Strip + Fantasme\n👕 Enlève un vêtement (ou accessoire) lentement en décrivant le fantasme que tu n’as encore jamais osé réaliser." },
            { text: "Gage : Le Toucher Aveugle\n👁️‍🗨️ Les yeux bandés, tu dois identifier un joueur uniquement en touchant son visage. Si tu te trompes, tu dois faire un compliment sensuel à chaque personne dans la pièce." }
        ],
        4: [
            { text: "👄 Gage: Murmure interdit\n💬 Approche-toi d’un joueur… et murmure dans son oreille le mot ou la phrase la plus cochonne que tu connaisses, de plus en plus lentement." },
            { text: "👅 Gage: Le Baiser Interdit\n💋 Tu dois embrasser le joueur de ton choix… sauf sur la bouche, et avec les yeux fermés. Si tu tombes sur un endroit “tabou”, tu recommences." },
            { text: "💦 Gage: L’Explorateur silencieux\n😶 Ferme les yeux. Laisse un joueur·se de ton choix te toucher pendant 10 secondes. À toi de deviner avec quelle partie de son corps il ou elle t’a effleuré." }
        ]
    }
};

